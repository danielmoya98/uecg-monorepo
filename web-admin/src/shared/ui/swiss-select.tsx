...
export function SwissSelect({
  ...
  // Determinamos si un filtro está activo (no es valor por defecto ni "TODOS")
  const normalizedValue = (value || '').toUpperCase()
  const isActive = Boolean(value) && normalizedValue !== 'TODOS' && normalizedValue !== 'ALL' && normalizedValue !== 'TODAS' && value !== ''
  ...
  return (
    <div className={`relative min-w-[190px] flex-1 md:flex-none ${className}`} ref={ref}>
      <button
        ...
        className={`w-full h-full flex items-center justify-between border bg-white px-4 py-3 text-[11px] font-bold uppercase tracking-widest transition-colors shadow-sm focus:outline-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed ${
          isActive
      ...
      >
        <div className="flex items-center gap-2 truncate pr-2">
          <Icon className={`w-3.5 h-3.5 shrink-0 ${isActive ? 'text-uecg-blue' : 'text-uecg-gray'}`} />
      ...
      {isOpen && (
        <div
          role="listbox"
          className="absolute top-full left-0 mt-1 w-full bg-white border border-uecg-line shadow-xl z-40 animate-in fade-in slide-in-from-top-1 duration-200"
        >
